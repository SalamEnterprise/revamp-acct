...
| From                                             | To                                    | Key                               | Purpose |
| ------------------------------------------------ | ------------------------------------- | --------------------------------- | ------- |
| `txn_source` → `je_header_staging`               | `source_rowid`                        | Trace transaction origin          |         |
| `je_header_staging` → `ledger_entry_header`      | `je_number`, `run_id`                 | Transition from staging to posted |         |
| `je_line_staging` → `ledger_entry_line`          | `je_internal_id` → `je_id`            | Maintain line-level trace         |         |
| `ledger_entry_line` → `account_balance_snapshot` | `account_code`, `fund_code`, `period` | Summarize financial position      |         |
...
flowchart TD
    ...
    subgraph "6. Governance & Audit Trail"
        ...
        G3["Data Lineage: source_rowid ↔ JE ↔ Ledger ↔ Report"]
    end
